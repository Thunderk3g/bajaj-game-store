...
import { GameState, UserLead } from './types';
...
const App: React.FC = () => {
  ...
  const [leadInfo, setLeadInfo] = useState<UserLead | null>(null);
  ...
  const handleLeadSubmit = async (lead: UserLead) => {
    ...
    const result = await submitToLMS({
      ...
      score,
      summary_dtls: 'FutureGuard Child Plan Lead',
    });
    const returnedLeadNo = (result.leadNo || result.LeadNo || null) as string | null;
    if (returnedLeadNo) {
      setLeadNo(returnedLeadNo);
      sessionStorage.setItem('futureguardChildLeadNo', returnedLeadNo);
    }
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4">
      <div className="w-full max-w-md bg-white rounded-3xl shadow-xl overflow-hidden relative border border-slate-100 min-h-[600px] flex flex-col">
        ...
        <div className="flex-grow p-6 flex flex-col items-center justify-center">
          ...
          {gameState === GameState.LEAD_FORM && <LeadCapture onSubmit={handleLeadSubmit} />}
      ...
      {showSlotBooking && leadInfo && (
        <SlotBooking
          ...
          remarks="Slot Booking via FutureGuard Child"
          onComplete={() => {
