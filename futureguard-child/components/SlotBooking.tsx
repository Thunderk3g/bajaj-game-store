...
const SlotBooking: React.FC<Props> = ({
  ...
  const handleBook = async () => {
    ...
    try {
      ...
      else setError(result.error || 'Could not book your slot. Please try again.');
    } finally {
      setSubmitting(false);
    }
  };
        ...
        <div className="space-y-3">
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 focus:ring-2 focus:ring-indigo-500 focus:outline-none"
            />
          </div>
          ...
          <button
            onClick={handleBook}
            disabled={submitting}
            className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 rounded-xl transition-all active:scale-95 disabled:opacity-60"
          >
