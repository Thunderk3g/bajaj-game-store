// LMS integration for FutureGuard: Child Plan Challenge
...
export const submitToLMS = async (data: LmsLeadData): Promise<LmsResponse> => {
  ...
  const payload = {
    ...
    userId,
    gameID,
    remarks: `Game: ${gameID}${data.score != null ? ` | Score: ${data.score}` : ''} | ${data.summary_dtls || 'FutureGuard Child Lead'}`,
...
export const updateLeadNew = async (
  ...
  const payload = {
    ...
    miscObj1: {
      ...
      stringval6: data.remarks || 'Slot Booking via FutureGuard Child',
