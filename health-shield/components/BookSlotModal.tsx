...
import { BLUE, BOOK_SLOT_TIMES, TOTAL_BRICKS, COVERAGE_WEIGHT, LIVES_BONUS_MAX, MAX_LIVES } from '../constants';
...
const BookSlotModal: React.FC<Props> = ({ name, mobile, onClose, onBook, result }) => {
  ...
  async function handleBook() {
    ...
    try {
      const leadNo = sessionStorage.getItem('healthShieldLeadNo');
      let finalScore = 0;
      if (result) {
      ...
      if (leadNo) {
        await updateLeadNew(leadNo, {
          ...
          date: date,
          time: time,
          remarks: `Health Shield - Appointment | Score: ${finalScore}`
        });
      } else {
        ...
        await submitToLMS({
          ...
          date: date,
          timeSlot: time,
          summary_dtls: 'Health Shield - Appointment'
        });
      }
      onBook();
    } catch (err) {
