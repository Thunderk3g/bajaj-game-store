...
const BookSlotModal: React.FC<Props> = ({ name, mobile, leadNo, onClose, onBook }) => {
    ...
    try {
      if (leadNo) {
        await updateLeadNew(leadNo, {
          ...
          date,
          time,
          remarks: 'Slot Booking via Health Shield',
        });
      }
    } catch (err) {
      // eslint-disable-next-line no-console
      console.error('updateLeadNew failed', err);
    } finally {
      setSubmitting(false);
      onBook();
    }
      ...
      <div className="bg-white w-full max-w-sm rounded-t-3xl max-h-[92vh] overflow-y-auto pop">
        ...
        <div className="px-6 pb-8">
          ...
          <button
            ...
            style={{
              background: BLUE,
              boxShadow: `0 4px 16px rgba(0,61,166,0.35)`,
