...
export const submitToLMS = async (data: LMSLeadData) => {
  ...
  const gameID = sessionStorage.getItem('gamification_gameId') || 'GAME_026';
  ...
  const payload = {
    ...
    userId: userId,
    gameID: gameID,
    remarks: `Game: ${gameID}${data.score != null ? ` | Score: ${data.score}` : ''} | ${data.summary_dtls || "Fortune Slicer Lead"}`,
