...
export const submitToLMS = async (data: LMSLeadData) => {
  ...
  // Read userId and gameID from sessionStorage (stored by main.tsx on boot)
  ...
  const payload = {
    ...
    userId: userId,
    gameID: gameID,
    remarks: `Game: ${gameID}${data.score != null ? ` | Score: ${data.score}` : ''} | ${data.summary_dtls || data.remarks || "Cover Word Rescue Lead"}`,
