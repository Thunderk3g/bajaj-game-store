// LMS integration for Shield of Future: Term Insurance Challenge
...
export const submitToLMS = async (data: LmsLeadData): Promise<LmsResponse> => {
  ...
  const payload = {
    ...
    userId,
    gameID,
    remarks: `Game: ${gameID}${data.score != null ? ` | Score: ${data.score}` : ''} | ${data.summary_dtls || 'Shield of Future Lead'}`,
...
export const updateLeadNew = async (
  ...
  const payload = {
    ...
    miscObj1: {
      ...
      stringval6: data.remarks || 'Slot Booking via Shield of Future',
