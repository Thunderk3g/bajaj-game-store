...
export interface LMSUpdateData {
  name?: string;
  firstName?: string;
  lastName?: string;
  mobile?: string;
...
export const submitToLMS = async (data: LMSLeadData) => {
  const UAT_URL = "https://sales.bajajlife.com/BalicLmsUtil/whatsappInhouse";

  const userId = sessionStorage.getItem('gamification_userId') || '';
  const gameID = sessionStorage.getItem('gamification_gameId') || 'child-plans-future-sprint';
  ...
  const payload = {
    ...
    userId: userId,
    gameID: gameID,
    remarks: `Game: ${gameID}${data.score != null ? ` | Score: ${data.score}` : ''} | ${data.summary_dtls || "Future Sprint Lead"}`,
...
export const updateLeadNew = async (leadNo: string, data: LMSUpdateData) => {
  const UAT_URL = "https://sales.bajajlife.com/BalicLmsUtil/updateLeadNew";
