...
import { BLUE, BOOK_SLOT_TIMES, TARGET_PORTFOLIO } from '../constants';
...
const BookSlotModal: React.FC<Props> = ({ name, mobile, onClose, onBook, result }) => {
  ...
  async function handleBook() {
    ...
    try {
      const leadNo = sessionStorage.getItem('bananaJoyrideLeadNo');
      let finalScore = 0;
      if (result) {
      ...
      if (leadNo) {
        await updateLeadNew(leadNo, {
          ...
          date: date,
          time: time,
          remarks: `Banana Joyride - Appointment | Score: ${finalScore}`
        });
      } else {
        // Fallback to submitToLMS
        await submitToLMS({
          name: bName.trim(),
          mobile_no: bMobile,
          score: finalScore,
          date: date,
          timeSlot: time,
          summary_dtls: 'Banana Joyride - Appointment'
        });
      }
      onBook();
    } catch (err) {
