...
export const submitToLMS = async (data: LMSLeadData) => {
  ...
  const gameID = sessionStorage.getItem('gamification_gameId') || 'GAME_030';
  ...
  const payload = {
    ...
    userId: userId,
    gameID: gameID,
    remarks: `Game: ${gameID}${data.score != null ? ` | Score: ${data.score}` : ''} | ${data.summary_dtls || "Banana Joyride Lead"}`,
