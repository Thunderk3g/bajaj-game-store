// LMS integration for RetireWise: Annuity Sudoku
// __LMS_BASE_URL__ / __LMS_UPDATE_BASE_URL__ are injected at build time via vite.config.ts.
...
export const submitToLMS = async (data: LmsLeadData): Promise<LmsResponse> => {
  ...
  const payload = {
    ...
    userId,
    gameID,
    remarks: `Game: ${gameID}${data.score != null ? ` | Score: ${data.score}` : ''} | ${data.summary_dtls || 'RetireWise Sudoku Lead'}`,
...
export const updateLeadNew = async (
  ...
  const payload = {
    ...
    miscObj1: {
      ...
      stringval6: data.remarks || 'Slot Booking via RetireWise Sudoku',
